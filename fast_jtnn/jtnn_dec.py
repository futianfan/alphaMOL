...
class JTNNDecoder(nn.Module):
    ...
    def forward(self, mol_batch, x_tree_vecs, x_mol_vecs, alpha):
        ...
        #print("pred_hiddens shape", pred_hiddens.shape)  ## (419, 300)    (396, 300)
        #print("pred_contexts shape", pred_contexts.shape)  ## (419,)       (396,) 
        ...
        pred_loss = self.pred_loss(pred_scores, pred_targets) / len(mol_batch)  ## (1,)
        ...
        print("pred_contexts", pred_contexts.shape, pred_targets.shape) 
        print("stop_contexts", stop_contexts.shape, stop_targets.shape)  
        #import pickle      
        #pickle.dump((pred_targets, pred_scores, stop_targets, stop_scores, pred_contexts, stop_contexts), open("tmp.pkl", "wb"))
        #print("done")
        #exit()
    ...
    def reward_decode(self, x_tree_vecs, x_mol_vecs):
        ...
        #Root Prediction
        root_score = self.attention(init_hiddens, contexts, x_tree_vecs, x_mol_vecs, 'word')
        ## print "root_score shape", root_score.shape ## (1,780)
        _,root_wid = torch.max(root_score, dim=1)
        root_wid = root_wid.item()

        log_pi, _ = torch.max(torch.nn.functional.log_softmax(root_score, dim = 1), dim = 1)
        #print("root1: log_pi shape", log_pi.shape, log_pi)  ### (1,)
        log_pi = log_pi.sum()
        #print("root2: log_pi shape", log_pi.shape, log_pi)  ### (1,)
        ...
        for step in xrange(MAX_DECODE_LEN):
            ...
            stop_score = self.attention(stop_hiddens, contexts, x_tree_vecs, x_mol_vecs, 'stop')
            ###
            #print "stop_score ", stop_score, stop_score.shape ### (1,1)
            log_sigmoid_stop_score = torch._C._nn.log_sigmoid(stop_score)
            assert log_sigmoid_stop_score.sum().requires_grad 
            #print "stop_score1: log_pi", log_sigmoid_stop_score, log_sigmoid_stop_score.shape
            log_pi += log_sigmoid_stop_score.sum()
            log_sum_stop_score = log_sum_stop_score + log_sigmoid_stop_score.sum() \
                 if 'log_sum_stop_score' in locals() or 'log_sum_stop_score' in globals() \
                 else log_sigmoid_stop_score.sum()
            ...
            if not backtrack: #Forward: Predict next clique
                ...
                pred_score2 = torch.nn.functional.log_softmax(pred_score, dim = 1)
                log_softmax_score,sort_wid = torch.sort(pred_score2, dim=1, descending=True)
                #print "log_softmax_score", log_softmax_score.requires_grad, log_softmax_score.shape  ### (1, 780)  
                sort_wid = sort_wid.data.squeeze()

                next_wid = None
                #for wid,logscore in zip(sort_wid[:5], sort_score[:5]):
                for i in range(5):
                    wid = sort_wid[i]
                    slots = self.vocab.get_slots(wid)
                    node_y = MolTreeNode(self.vocab.get_smiles(wid))
                    if have_slots(fa_slot, slots) and can_assemble(node_x, node_y):
                        next_wid = wid
                        next_slots = slots
                        #print "next clique: logscore", logscore.shape, logscore.requires_grad
                        logscore = log_softmax_score[0,i]
                        assert logscore.requires_grad
                        log_pi += logscore 
                        break
        ...
        return root, all_nodes, log_pi, log_sum_stop_score, molecule_size  
