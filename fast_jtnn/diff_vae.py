...
import copy, math
from props import similarity, drd2, qed, penalized_logp 
import eventlet
time_limit = 10
...
mol_size_diff = 3 
#mol_size_fnc = lambda x,y: 0 if x-y>-mol_size_diff and x-y<mol_size_diff else abs(x-y)-mol_size_diff
mol_size_fnc = lambda x,y: abs(x-y) - mol_size_diff
...
class DiffVAE(nn.Module):
    ...
    def reward_decode(self, x_tree_vecs, x_mol_vecs):
        ## batch size = 1
        assert x_tree_vecs.size(0) == 1 and x_mol_vecs.size(0) == 1
        pred_root,pred_nodes,log_pi,log_sum_stop_score,molecule_size \
                 = self.decoder.reward_decode(x_tree_vecs, x_mol_vecs)
        #print "terminate decoder.reward_decode"
        if len(pred_nodes) == 0: 
            return None, None
        elif len(pred_nodes) == 1: 
            return pred_root.smiles, log_pi
        ...
        #print "terminate diff_vae:reward_decode"
        return Chem.MolToSmiles(cur_mol) if cur_mol is not None else None,\
                 log_pi, log_sum_stop_score, molecule_size


    def forward(self, x_batch, y_batch, beta, alpha):
        ...
        #import pickle 
        #pickle.dump((y_batch, x_tree_vecs, x_mol_vecs), open("tmp.pkl", 'wb'))
        word_loss, topo_loss, word_acc, topo_acc = self.decoder(y_batch, x_tree_vecs, x_mol_vecs, alpha)
    ...
    def forward_reward(self, smiles, x_batch, \
                similarity_threshold=0.3, similarity_weight=0.0, \
                drd_threshold = 0.0, drd_weight = 0.0, \
                qed_threshold = 0.0, qed_weight = 0.0, \
                molecule_size_weight = 1e-3, molecule_size_threshold = 15):
        '''
            output **reward** is pseudo-objective in RL that can be back-propagated directly. 
        '''
        x_batch, x_jtenc_holder, x_mpn_holder = x_batch
        x_tree_vecs, _, x_mol_vecs = self.encode(x_jtenc_holder, x_mpn_holder)
        #print "forward x_tree_vecs shape", x_tree_vecs.shape ## (32, 23, 300)
        #print "forward x_mol_vecs shape", x_mol_vecs.shape  ## (32, 32, 300) 
        for i in range(x_tree_vecs.size(0)):
            x_tree_vec = x_tree_vecs[i:i+1,:,:] 
            x_mol_vec = x_mol_vecs[i:i+1,:,:] 
            z_tree_vecs, z_mol_vecs = self.fuse_noise(x_tree_vec, x_mol_vec)
            try:
                new_smiles, log_pi, log_sum_stop_score, molecule_size \
                 = self.reward_decode(z_tree_vecs[0].unsqueeze(0), z_mol_vecs[0].unsqueeze(0))
            except:
                new_smiles = None 
            '''try:
                with eventlet.Timeout(time_limit,False):
                    new_smiles, log_pi = self.reward_decode(z_tree_vecs[0].unsqueeze(0), z_mol_vecs[0].unsqueeze(0))
            except: 
                new_smiles = None
            '''
            #print "log_pi", log_pi, "new_smiles", new_smiles
            if new_smiles is None:
                continue
            input_smile = smiles[i]
            sim_value = similarity(input_smile, new_smiles) - similarity_threshold
            drd_score = drd2(new_smiles) - drd_threshold
            mol_in = MolTree(input_smile)
            mol_out = MolTree(new_smiles)
            mol_size_reward = mol_size_fnc(mol_in.size(), mol_out.size())

            #qed_score = qed(new_smiles) - qed_threshold
            #print "similarity", sim_value 
            return log_pi * float(sim_value * similarity_weight + drd_weight * drd_score) \
                 + molecule_size_weight * mol_size_reward * log_sum_stop_score 
        return None
