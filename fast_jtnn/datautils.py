...
class PairTreeFolder2(object):
    ...
    def __iter__(self):
        for fn in self.data_files:
            ...
            dataset = PairTreeDataset2(batches, self.vocab, self.y_assm)
            dataloader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=self.num_workers, collate_fn=lambda x:x[0])
...
class PairTreeDataset2(Dataset):
    ...
    def __getitem__(self, idx):
        smiles, batch0, batch1 = zip(*self.data[idx])
        return smiles, tensorize(batch0, self.vocab, assm=False), tensorize(batch1, self.vocab, assm=self.y_assm)
